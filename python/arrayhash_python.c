...
/*
 * TODO FIXME The ArrayHash API doesn't currently support anyway to signal
 *            failure in encoding to the upper layers. When it does
 *            support signalling, fix this.  Places that need error
 *            handling are all the Py_Get* functions, they can all
 *            return NULL
...
encode_get_type(void *data)
{
	...
	if (PyDict_Check(o)) {
		...
		/* Would be nice to be able to encode more types. */
		return STATE_SCALAR;
	}
}
...
encode_get_scalar(void *data, size_t *len)
{
	...
	if (s != NULL) {
		/*
		 * It might not be readily apparent why this (Not calling
		 * INCREF and passing the internal string) is safe. We send
		 * the string up to the caller, who is then going to
		 * immediately serialize it to a buffer. This all happens
		 * inside of a call where the Python GC can't free it. This
		 * is probably obvious to everyone else, but I have to
		 * think about it each time through this function.
		 */
...
{
	struct enc_list_iter	*iter;

	/*
	 * TODO FIXME The ArrayHash API doesn't currently support anyway
	 *            to signal failure in encoding to the upper
	 *            layers. When it does support signalling, fix
	 *            this.
	 */
	...
	if (!*ret) {
		*ret = PyString_FromStringAndSize(str, len);
		...
		D(fprintf(stderr, "a string_append_empty(%p): %d ::'%s', %d\n",
		    *ret, ((PyObject *) *ret )->ob_refcnt, str, len));
		return;
	}
...
list_end(ssp_val *ret)
{
	D(fprintf(stderr, "list_end(%p) counts: %d\n = %p %d\n", *ret,
	    list->ob_refcnt, *ret , PyList_Size((PyObject *)*ret)));
}
...
map_element(ssp_val *ret, ssp_val *key, ssp_val *val)
{
	...
	D(fprintf(stderr, "map_element(%p, %p, %p) counts: %d %d %d\n", *ret,
	    *key, val?*val:0, dict->ob_refcnt, ((PyObject *) *key)->ob_refcnt,
